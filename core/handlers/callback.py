...
#     answer = f'{call.message.from_user.first_name}, ты выбрал Apple Macbook {model} с диагналью {size} ' \
#              f'с процессором {chip} {year} года'
...
async def select_macbook(call: CallbackQuery, bot:Bot, callback_data: MacInfo):
    ...
    answer = f'Ты выбрал Apple Macbook {model} с диагналью {size} ' \
             f'с процессором {chip} {year} года'
    await call.message.answer(answer)
    await call.answer()